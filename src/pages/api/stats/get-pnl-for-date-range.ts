  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      for (let i = 0; i < dates.length; i++) {
        ...
        const dailyRecaps = (await googleCloudStorageClient.readJsonFile(`recaps/next/${date}.json`) as object[]);
        if (!dailyRecaps) {
          return res.status(404).json({
            message: `${date} recaps not found`,
          });
        }
        const freeRecaps = dailyRecaps.filter((r) => r.plan === 'free');
        const premiumRecaps = dailyRecaps.filter((r) => r.plan === 'premium');

        const allGuessed = dailyRecaps.filter((r) => r.is_guessed === 'YES');
        const freeGuessed = dailyRecaps.filter((r) => r.is_guessed === 'YES' && r.plan === 'free');
        const premiumGuessed = dailyRecaps.filter((r) => r.is_guessed === 'YES' && r.plan === 'premium');
